        ...
        request.onreadystatechange = function (response) {
            if (this.readyState === 4) {
                if (this.status === 201) {
                    ...
                    reject(
                        myModal(4000, 'Une erreur est survenue, merci de réessayer ultérieurement', 'https://thumbs.gfycat.com/DiligentWastefulGavial-small.gif'), setTimeout(function(){location.reload()}, 3000)
                    );
                }
            }
        };
...
if (productsAddedToCart !== null) { //If there is product(s) added in localStorage
                ...
                <form action="" id="form1">\n
                    ...
                    <div class="formCss">\n
                        <label for="address">Adresse : </label>\n
                            <input type="text" name="address" id="address" class='infoRequired' required placeholder='3 rue des lilas, ...' pattern='^[0-9]{1,4}[ ,-][ A-Za-zÀ-ÿ0-9\-]+$' title='Champ Obligatoire. Renseignez ce champ au format : 3 rue des lilas, ou 3, rue des Lilas' onkeyup="this.value = this.value.toLowerCase();">\n
                    </div>\n
                    <div class="formCss">\n
    ...
    document.getElementById('btn-submit').addEventListener('click', function (event) {
        ...
        if (!document.getElementById('form1').checkValidity()) {
            ...
            myModal(2500, 'Merci de renseigner tous les champs du formulaire !', 'https://media1.tenor.com/images/8bce66c7f447d8b7cfce2cfe1da61782/tenor.gif?itemid=11295732');
        } else {
